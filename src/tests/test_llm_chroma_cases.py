...
def check_question_grammar(client: OpenAI, question: str, *, model: str) -> Dict[str, Any]:
    system = (
        "Ты - строгий корректор русского языка. "
        "Твоя задача - проверить вопрос на ошибки ТОЛЬКО в согласовании слов в предложении."
        "Не придирайся к разговорности, частицам, мату, стилистике, разговорным словам, структуре."
    )
    user = (
        "Вопрос:\n"
        f"{question}\n\n"
        ...
        '  "error_spans": ["короткие фрагменты с ошибкой (как в тексте)"],\n'
        '  "why": "коротко, в 1-2 фразах"\n'
        "}\n\n"
        "Правила:\n"
        "- has_error=true только если ошибка реально режет глаз.\n"
        "- Если ошибка сомнительная - has_error=false.\n"
    )
    ...
    try:
        data = json.loads(raw)
        if isinstance(data, dict) and "has_error" in data:
            return data
    except Exception:
        pass
    return {"has_error": False, "error_spans": [], "why": "parse_failed", "raw": raw}
...
def main() -> None:
    ...
    for temp in TEMPS:
        ...
        for fp in files:
            ...
            judge = check_question_grammar(client, q, model=LLM_MODEL)
            q_flagged = f"{q} ошибка" if judge.get("has_error") else q
            ...
            dump_debug_json(
                debug_dir / f"{fp.stem}.debug.json",
                {
                    ...
                    "grammar_judge": judge,
                    "generated_question_flagged": q_flagged,
                },
            )
            ...
            out_rows.append(
                {
                    ...
                    "generated_question": q,
                    "generated_question_flagged": q_flagged,
                    "grammar_has_error": bool(judge.get("has_error")),
                    "grammar_why": judge.get("why", ""),
                }
            )
